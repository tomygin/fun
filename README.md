...
一门比 Lua 还小的编程语言 —— 实现约 3,600 行 Go（加测试约 5,200 行），语法借鉴 Go 与 Python，
唯一的数据结构 `{ }` 借鉴 Lua。
...
总之，这是一门比 lua 还小的语言，实现目前约 3,600 行 Go 代码（源码加测试约 5,200 行），希望你玩得开心。
...
- **语法混血**：`:=` 声明来自 Go，`for` 是唯一的循环关键字（同样来自 Go），
  真值/短路逻辑的味道来自 Python。
- **数字不会骗你**：三层数字塔（int64 → 定点小数 → 大有理数）按需自动
  切换 —— `0.1 + 0.2 == 0.3` 为真，整数溢出自动提升大数永不回绕，
  常规运算保持机器速度。
...
| 类型 | 例子 |
| --- | --- |
| 数字 | `1`、`-3`、`3.14` —— 十进制精确，`0.1 + 0.2 == 0.3` 为真，详见下节 |
...
Fun 不用二进制浮点做算术。数字内部是一座**三层数字塔**，按需自动切换，
语法完全无感知（完整演示见 [`__example/precision.fun`](__example/precision.fun)）：

| 层 | 表示 | 何时使用 | 速度 |
| --- | --- | --- | --- |
| 整数 | int64 + 溢出检测 | 整数字面量与运算 | 机器速度 |
| 定点小数 | 十进制系数 + 小数位 | 常规小数（`0.1` 就是精确的 1/10） | 机器速度 |
| 大有理数 | `big.Rat` | 溢出提升 / 除不尽 | 慢，但精确 |

由此获得的保证：

```js
0.1 + 0.2 == 0.3            // true —— 不是 0.30000000000000004
0.1 * 0.2                   // 0.02
9007199254740992 + 1        // 9007199254740993 —— 不丢位（浮点语言在 2^53 后丢）
9223372036854775807 + 1     // 9223372036854775808 —— int64 溢出自动提升大数，永不回绕
1 / 4                       // 0.25 —— 除得尽降回快层
1 / 3 * 3 == 1              // true —— 1/3 内部是精确有理数
0.5 + 0.5                   // 1 —— 自动归一化降回整数
1 / 0                       // 抛出错误（用 try 捕获），% 0 同理
```

**精度的明确承诺（写进文档的规格）**：

- 小数按**十进制**精确存储与运算，不存在二进制浮点误差。
- 定点快路径容纳约 **18 位有效数字**；超出自动切换到大有理数，**仍然精确**，
  只是变慢。
- 整数运算带溢出检测，溢出自动提升为任意精度大数，**永不回绕、永不丢位**。
- 唯一的"近似"发生在**无限循环小数的打印**：四舍五入到小数点后 **20 位**
  （如 `1/3` 打印为 `0.33333333333333333333`）；内部值保持精确，
  后续运算不受打印影响。
- 性能代价只由用到的层承担：整数与常规小数全程走 int64 快路径
  （实测一百万次 `0.1` 累加约 0.6s，结果精确等于 `100000`）；
  只有大数和无限循环小数才落到慢层。
...
- `parser/`：左递归下降，把 token 变成 `[]any` 形式的 s-expression，
  ...
  元编程空间，见"元编程"一节）。
- `number/`：三层数字塔（int64 / 定点小数 / 大有理数），保证十进制精确
  与永不溢出，同时让常规运算停在 int64 快路径（见"数字与精度"一节）。
